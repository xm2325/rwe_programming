...
def run_qc_registry(config: StudyConfig = DEFAULT_CONFIG) -> list[QCCheck]:
    ...
    return [
        QCCheck("QC001", "Expected source patient count", len(raw) == config.n_patients, str(len(raw)), str(config.n_patients)),
        QCCheck("QC002", "Patient IDs are unique", raw.patient_id.is_unique, str(raw.patient_id.nunique()), str(len(raw))),
        QCCheck("QC003", "No missing patient IDs", raw.patient_id.notna().all(), str(int(raw.patient_id.isna().sum())), "0"),
        QCCheck("QC004", "Exposure is binary", set(raw.ucg.unique()).issubset({0, 1}), str(sorted(raw.ucg.unique().tolist())), "[0, 1]"),
        QCCheck("QC005", "Follow-up is positive", bool((raw.followup_years > 0).all()), f"min={raw.followup_years.min():.6f}", ">0"),
        QCCheck("QC006", "Follow-up within configured horizon", bool((raw.followup_years <= config.max_followup_years).all()), f"max={raw.followup_years.max():.6f}", f"<={config.max_followup_years}"),
        ...
        QCCheck("QC009", "Post-weighting balance threshold", max_smd < config.balance_threshold_abs_smd, f"max abs SMD={max_smd:.6f}", f"<{config.balance_threshold_abs_smd}"),
        QCCheck("QC010", "Effective sample size remains substantial", ess >= 0.70 * len(df), f"ESS={ess:.1f}", f">={0.70 * len(df):.1f}"),
        QCCheck("QC011", "Attrition ledger reconciles to final cohort", int(attrition.iloc[-1].included_n) == len(raw), str(int(attrition.iloc[-1].included_n)), str(len(raw))),
        QCCheck("QC012", "Primary outcome is binary", set(raw.ckd_event.unique()).issubset({0, 1}), str(sorted(raw.ckd_event.unique().tolist())), "[0, 1]"),
    ]
