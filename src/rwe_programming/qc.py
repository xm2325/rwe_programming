...
def run_qc_registry(config: StudyConfig = DEFAULT_CONFIG) -> list[QCCheck]:
    ...
    return [
        QCCheck("QC001", "Expected source patient count", bool(len(raw) == config.n_patients), str(len(raw)), str(config.n_patients)),
        QCCheck("QC002", "Patient IDs are unique", bool(raw.patient_id.is_unique), str(raw.patient_id.nunique()), str(len(raw))),
        QCCheck("QC003", "No missing patient IDs", bool(raw.patient_id.notna().all()), str(int(raw.patient_id.isna().sum())), "0"),
        QCCheck("QC004", "Exposure is binary", bool(set(raw.ucg.unique()).issubset({0, 1})), str(sorted(raw.ucg.unique().tolist())), "[0, 1]"),
